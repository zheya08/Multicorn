...
``dirname`` (required)
  The full path to the directory of CSV file containing the data. This file must be readable
...
class MultiCsvFdw(ForeignDataWrapper):
    ...
    def __init__(self, fdw_options, fdw_columns):
        super(MultiCsvFdw, self).__init__(fdw_options, fdw_columns)
        self.dirname = fdw_options["dirname"]
    ...
    def execute(self, quals, columns):
        filename_list = os.listdir(self.dirname)

        for each in filename_list:
            filename = os.path.join(self.dirname, each)
